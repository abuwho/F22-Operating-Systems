...
void *prime_counter(void *arg)
{
    ...
    */
    ...
    int * re_turn = malloc(sizeof(int));
    *re_turn = primes_count_in_interval(start, finish);
    return re_turn;
}
...
int main(int argc, char *argv[])
{
    ...
    /*
    TODO
    Complete this loop. You shall spawn <n_threads> threads, each of which computes the number
    of primes in a segment of length <segment_size>. The union of those intervals shall
    correspond to the interval [0, n).

    First, initialize the value of requests[i] with a simple mathematical computation. Be
    careful not to overrun the last interval.

    Second, spawn a thread that takes requests[i] as an argument. Be careful how you pass the
    arguments.
    */
    
    requests[i].start = i * segment_size;
    requests[i].finish = i * segment_size + segment_size;
